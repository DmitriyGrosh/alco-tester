...
export class Http extends Network {
  ...
  private async executeRequest<T>(
    ...
    if (this.getBearerToken) {
      ...
      bearerToken =
        tokenResult instanceof Promise ? await tokenResult : tokenResult;
    }
  ...
  private async handleUnauthorized(
    triggeringRequest: QueuedRequest,
  ): Promise<void> {
    if (this.isRefreshing) {
    ...
    ].filter((req) => !req.externalSignal?.aborted);
    ...
    const retryPromises = requestsToRetry.map((request) =>
  ...
  private async retryRequest(request: QueuedRequest): Promise<void> {
    try {
      ...
      // Convert existing headers to plain object if needed
      const existingHeaders =
        optionsWithoutSignal.headers instanceof Headers
      ...
      const response = await super
        .requestRaw(request.url, retryOptions)
        .catch((error) => {
